...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    # 如果有标签，绘制标签
    if label:
        font_scale = 0.5  # 调整字体大小
        font_thickness = 1
        text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness)[0]
        text_origin = (int(x[0]), int(x[1]) - 5)  # 标签位置在框的上方

        # 绘制背景矩形
        cv2.rectangle(img, (text_origin[0], text_origin[1] - text_size[1]),
                      (text_origin[0] + text_size[0], text_origin[1]), color, -1)
